...
<div>Вы закончили редактировать курс? <button @click="SendEditTest">{{visiblesendedit}}</button></div>

<div class="accordion accordion-flush" id="accordionFlushExample">
  <div class="accordion-item" v-for="test in daatforedittest" :key="test">
    <h2 class="accordion-header" id="flush-headingOne">
      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseOne" aria-expanded="false" aria-controls="flush-collapseOne">
         Вопрос <input class="form-control" type="text" v-model="test.quest">
      </button>
    </h2>
    <div id="flush-collapseOne" class="accordion-collapse collapse" aria-labelledby="flush-headingOne" data-bs-parent="#accordionFlushExample">
      <div class="accordion-body">

          <div  class="border border-dark p-2" v-for="anserwedit in test.anserws" :key="anserwedit">
    Вариант ответа
    <input class="form-control" type="text" v-model="anserwedit.q">
Правильный Ответ
    <input  type="checkbox" v-model="anserwedit.t">
    {{ anserwedit.t}}
    </div>
...
export default {
    ...
    data(){
        return{
            visiblesendedit: 'ДАА!',
            daatforedittest: JSON.parse(this.testItemsData),
            quest: null,
            anserw: null,
            anserwtrue: false,
            anserws: [],
    ...
    methods:{
        SendEditTest(){
            this.visiblesendedit = 'Данные отправлены!'
            this.$emit('UpdateTest',this.daatforedittest )
        },
        addTestMoodule(){
            if( !this.quest ){
            ...
            this.daatforedittest.push(testdata)
            this.quest = ''
            this.anserws = []
